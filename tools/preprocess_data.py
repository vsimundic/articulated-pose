...
class PoseDataset():
    def __init__(self, root_dir, item, num_points=1024,  objs=[], add_noise=False, noise_trans=0, mode='train', refine=False, selected_list=None, is_debug=False):
        ...
        obj_category = item
        instances_per_obj = os.listdir(
            self.dataset_render + '/' + obj_category)
        ...
        for ins in instances_per_obj:
            ...
            for art_index in os.listdir(base_path):
                ...
                while 1:
                    ...
                    self.list_rgb.append(
                        sub_dir0 + '/rgb/{}.png'.format(input_line))
                    self.list_depth.append(
                        sub_dir0 + '/depth/{}.h5'.format(input_line))
                    self.list_label.append(
                        sub_dir0 + '/mask/{}.png'.format(input_line))
            ...
            if self.root_dir.split('/')[-1] == 'shape2motion':
                tree_urdf = ET.parse(
                    self.root_dir + "/urdf/" + obj_category + '/' + ins + "/syn.urdf")  # todo
            else:
                tree_urdf = ET.parse(
                    self.root_dir + "/urdf/" + obj_category + '/' + ins + "/mobility.urdf")  # todo
            ...
            for joint in root_urdf.iter('joint'):
                try:
                    ...
                    # example: from 'joint_0' it splits it and takes only '0'
                    index_joint = int(joint.attrib['name'].split('_')[-1])

                for origin in joint.iter('origin'):
                    list_xyz[index_joint] = [
                        float(x) for x in origin.attrib['xyz'].split()]
                    try:
                        list_rpy[index_joint] = [
                            float(x) for x in origin.attrib['rpy'].split()]
                    except KeyError as e:
                        list_rpy[index_joint] = [0., 0., 0.]

                for axis in joint.iter('axis'):
                    list_axis[index_joint] = [
                        float(x) for x in axis.attrib['xyz'].split()]
    ...
    def __preprocess_and_save__(self, index):
        ...
        h5_save_path = self.root_dir + '/hdf5/' + \
            obj_category + '/' + ins + '/' + art_status
        ...
        # pose infos
        pose_dict = self.meta_dict[obj_category][ins][art_status]['frame_{}'.format(
            frame_order)]
        ...
        for link in temp_num_parts_range:
            if self.dataset_name == 'shape2motion':
                ...
                parts_world_pos[link] = np.array(
                    pose_dict['obj'][link][4]).astype(np.float32)
                parts_world_orn[link] = np.array(
                    pose_dict['obj'][link][5]).astype(np.float32)
        ...
        for link in temp_num_parts_range:
            if link == 1 and num_parts == 2:
                ...
                parts_urdf_pos[link] = - \
                    np.array(urdf_dict['link']['xyz'][link])
        ...
        for k in temp_num_parts_range:
            ...
            center_world_orn = np.array(
                [center_world_orn[3], center_world_orn[0], center_world_orn[1], center_world_orn[2]])
            ...
            my_model2camera_mat = np.dot(
                my_world2camera_mat, my_model2world_mat)
        ...
        projected_map = np.stack(
            [u_map * w_channel, v_map * w_channel, depth, w_channel]).transpose([1, 2, 0])
        projected_map1 = np.stack(
            [u_map * w_channel, v1_map * w_channel, depth, w_channel]).transpose([1, 2, 0])
        for s in temp_num_parts_range:
            ...
            projected_points = projected_map[choose_x[s][:].astype(
                np.uint16), choose_y[s][:].astype(np.uint16), :]
            projected_points = np.reshape(projected_points, [-1, 4])
            depth_channel = - projected_points[:, 3:4]
            cloud_cam = np.dot(projected_points[:, 0:2] - np.dot(
                depth_channel, projMat[0:2, 2:3].T), np.linalg.pinv(projMat[:2, :2].T))

            projected_points1 = projected_map1[choose_x[s][:].astype(
                np.uint16), choose_y[s][:].astype(np.uint16), :]
            projected_points1 = np.reshape(projected_points1, [-1, 4])
            cloud_cam_real = np.dot(projected_points1[:, 0:2] - np.dot(
                depth_channel, projMat[0:2, 2:3].T), np.linalg.pinv(projMat[:2, :2].T))
            cloud_cam_real = np.concatenate(
                (cloud_cam_real, depth_channel), axis=1)
            ...
            cloud_canon = np.dot(
                cloud_world, np.linalg.pinv(parts_model2world[s].T))
        ...
        for k in temp_num_parts_range:
            ...
            my_canon2urdf_r = euler_matrix(
                center_joint_orn[0], center_joint_orn[1], center_joint_orn[2])[:4, :4]  # [w, x, y, z]
            ...
            part_points_space = np.concatenate(
                (parts_cloud_canon[k], np.ones((parts_cloud_canon[k].shape[0], 1))), axis=1)
            parts_cloud_urdf[k] = np.dot(
                part_points_space, my_canon2urdf_mat.T)
        # >>>>>>>>>>>>>>> go to PNCS space
        for link in temp_num_parts_range:
            tight_w = max(parts_cloud_urdf[link][:, 0]) - \
                min(parts_cloud_urdf[link][:, 0])
            tight_l = max(parts_cloud_urdf[link][:, 1]) - \
                min(parts_cloud_urdf[link][:, 1])
            tight_h = max(parts_cloud_urdf[link][:, 2]) - \
                min(parts_cloud_urdf[link][:, 2])
            norm_factor = np.sqrt(1) / np.sqrt(tight_w **
                                               2 + tight_l**2 + tight_h**2)
            base_p = np.array([min(parts_cloud_urdf[link][:, 0]),  min(
                parts_cloud_urdf[link][:, 1]),  min(parts_cloud_urdf[link][:, 2])]).reshape(1, 3)
            extre_p = np.array([max(parts_cloud_urdf[link][:, 0]),  max(
                parts_cloud_urdf[link][:, 1]),  max(parts_cloud_urdf[link][:, 2])]).reshape(1, 3)
            center_p = (extre_p - base_p) / 2 * norm_factor

            parts_cloud_norm[link] = (parts_cloud_urdf[link][:, :3] - base_p) * norm_factor + \
                np.array([0.5, 0.5, 0.5]).reshape(
        ...
        if self.is_debug:  # or True:
            ...
            plt.show()
            plot3d_pts([parts_cloud_cam], [['part {}'.format(i) for i in range(
                len(parts_cloud_cam))]], s=5, title_name=['camera coords'])
            plot3d_pts([parts_cloud_world], [['part {}'.format(i) for i in range(
                len(parts_cloud_world))]], s=5, title_name=['world coords'])
            plot3d_pts([parts_cloud_canon], [['part {}'.format(i) for i in range(
                len(parts_cloud_canon))]], s=5, title_name=['canon coords'])
            plot3d_pts([parts_cloud_urdf], [['part {}'.format(i) for i in range(
                len(parts_cloud_urdf))]], s=5, title_name=['urdf coords'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='sapien',
                        help='name of the dataset we use')  # todo
    parser.add_argument('--item', default='drawer',
                        help='name of the dataset we use')
    parser.add_argument('--num_expr', default='0.01',
                        help='get configuration file per expriment')
    parser.add_argument('--mode', default='train',
                        help='indicating whether in demo mode')
    parser.add_argument('--debug', action='store_true',
                        help='indicating whether in debug mode')
    ...
    split_dataset(root_dset, [item], args, test_ins=infos.datasets[item].test_list,
                  spec_ins=infos.datasets[item].spec_list, train_ins=infos.datasets[item].train_list)
